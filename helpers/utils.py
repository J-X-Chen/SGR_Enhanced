...
def split_list(lst, n):
    for i in range(0, len(lst), n):
        yield lst[i:i + n]
    #yield 是一个关键字，用于定义生成器函数。每次调用生成器函数时，它会返回一个值，并在下次调用时从上次停止的地方继续执行
...
def extract_obs(obs: Observation,
    ...
    if not channels_last:
        ...
        new_obs_dict = {}  # 创建一个新字典来存储处理后的结果

        for k, v in obs_dict.items():
            # 检查值的类型是否为 NumPy 数组或列表
            if isinstance(v, np.ndarray) or isinstance(v, list):
                ...
                if isinstance(v, np.ndarray) and v.ndim == 3:
                    ...
                    new_obs_dict[k] = np.transpose(v, [1, 0])
                else:
                    # 如果不是三维数组，则增加一个维度
...
def create_obs_config(camera_names: List[str],
    ...
    #import rpdb; rpdb.set_trace()
    if method_name != 'SGR_Enhanced' or True:
        used_cams = CameraConfig(
            rgb=True,
            point_cloud=True,#cjx改，原本为True
            mask=False,
            depth=False,
        ...
        used_cams = CameraConfig(
            rgb=True,
            point_cloud=False,#cjx改，原本为True
            mask=False,
            depth=False,
            image_size=camera_resolution,
            render_mode=RenderMode.OPENGL)
    ...
    for n in camera_names:
        ...
        if method_name != 'SGR_Enhanced':
            cam_obs.append('%s_pointcloud' % n)
    if method_name == 'SGR_Enhanced':#这个设计好像没啥用
        cam_obs.append('pointcloud')
    #import rpdb; rpdb.set_trace()
