...
class SGRNetwork(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        '''
        输入处理: use_semantic和include_rgb参数控制是否使用语义和RGB图像作为输入。num_points和rand_sample参数控制输入点云的点数和采样方式.

        特征嵌入: proprio_emb_dim和rgb_emb_dim参数定义了本体感知信息和RGB图像的嵌入维度。语义特征维度:semantic_dim_per_layer参数控制每层语义信息的维度。

        网络结构: 层次化和重采样：hierarchical和resample参数决定了网络是否采用层次化结构和特征图的重采样策略。CLIP对齐：clip_align和align_type参数涉及CLIP对齐操作
        
        任务相关: 多任务学习：num_rotation_classes、num_grip_classes和num_collision_classes参数分别对应旋转、抓取和碰撞检测任务的类别数。点云预训练：point_pretrain_apply、point_pretrain_frozen和point_pretrain_lang参数控制点云预训练模型的应用和冻结策略。论文中可能提到了利用预训练的点云模型来初始化网络权重，这有助于模型快速收敛，并提高在有限样本下的性能。

        点云处理: 局部信息利用：point_relative_pos、trans_point_uniform_weight和rot_grip_collision_point_wise参数涉及到点云中局部信息的利用。为每个点分配权重或进行点-wise的预测
        '''
    ...
    def forward(self,
                pcd,#shape=(4,16,3,128,128)
                rgb,#shape同上
                bounds,#(16,6)每一行都是[-0.3000, -0.5000,  0.6000,  0.7000,  0.5000,  1.6000]
                proprio,#(16,4)
                lang_goal_emb,#(16,1024)
                lang_token_embs,#(16,77,512)
                feat_mask=None,
        ...
        xyz, feature = filter_and_sample_points(image_features, pcd, feat_size,
                                                ...
                                                self._resample,
                                                pcd_bound_masks)
        #TODO: 下一步研究pcd怎么来的，以及为什么rgb[0][:]是黑的(猜测为机械臂上的摄像头，第一帧被遮挡)
        # save
        import os

        # 指定文件夹名称
        saving_folder_name = "point_cloud_sample"
        ...
        for i in range(len(xyz)):
            point_tmp = xyz[i].cpu().permute(1,0)

            # 可视化点云
            fig = plt.figure()
            ax = fig.add_subplot(111, projection='3d')
            ax.scatter(point_tmp[0], point_tmp[1], point_tmp[2])
            ...
            # 保存图像到文件
            plt.savefig(os.path.join(saving_folder_name, "%d.png"%i))  # 保存为PNG格式
        


        #xyz.shape ---> torch.Size([16, 4096, 3])
        #feature.shape --->torch.Size([16, 4096, 259])
        ...
        if self._point_pretrain_apply:
            ...
            x = self.point_network(data,
                                   ...
                                   frozen_encoder=self._point_pretrain_frozen)#核心！！！
        else:
            data = {
                'pos': xyz.contiguous(),
            ...
            if self._late_proprio_concat:
                ...
                x = self.point_network(data, proprio2)#核心！！！
            else:
                x = self.point_network(data)#核心！！！
