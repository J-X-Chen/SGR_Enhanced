...
from agents.sgr_enhanced.sgr_network import SGRNetwork
from agents.sgr_enhanced.sgr_agent import SGRAgent
from agents.sgr_enhanced.stack_agent import StackAgent
...
def create_replay(batch_size: int,
    ...
    observation_elements.append(
        ObservationElement('low_dim_state', (LOW_DIM_SIZE, ), np.float32)) #(4, )

    # rgb, depth, point cloud, intrinsics, extrinsics
    for cname in cameras: #['front', 'left_shoulder', 'right_shoulder', 'wrist']
        observation_elements.append(
            ObservationElement('%s_rgb' % cname, (
                3,
                *image_size,
            ), np.float32))#(3, 128, 128)
        observation_elements.append(
            ObservationElement('%s_point_cloud' % cname, (3, image_size[0]*image_size[1]),
                               np.float32)#点云(3, 128, 128) #原本(3, *image_size)
        )  
    ...
    replay_buffer = TaskUniformReplayBuffer(
        save_dir=save_dir, #/home/kasm-user/saving/sgr/replay/close_jar/SGR_pointnext-xl_seg/sgrv2-demos_5-iter_20000/seed0
        ...
        update_horizon=1,
        observation_elements=observation_elements, #共27个元素
        extra_replay_elements=extra_replay_elements) #1个元素
...
def _add_keypoints_to_replay(cfg: DictConfig,
    ...
    for k, keypoint in enumerate(episode_keypoints):
        ...
        #import rpdb; rpdb.set_trace() #TODO：验为啥check33/32,点云要裁减，点云输入后维度可能不对
        timeout = False
    ...
    #import rpdb;rpdb.set_trace(port=4444) #TODO：验证一下rgb一样吗。先分析
...
def fill_replay(cfg: DictConfig,
    ...
    #=========================下载数据========================
    for variation_number in variation_numbers:#只有[0]
        for d_idx in range(num_demos): #num_demos=5
            # load demo from disk
...
def fill_multi_task_replay(
    ...
    processes = []
    n = np.arange(len(tasks)) #n表示任务个数
    split_n = utils.split_list(n, max_parallel_processes)#任务分割，max_parallel_processes--->3
    # import rpdb;rpdb.set_trace(port=4444)
    for split in split_n: #一个任务就不存在分割了
        for e_idx, task_idx in enumerate(split):
            ...
            p = Process(target=fill_replay,
                        args=(cfg, obs_config, rank, replay, task, num_demos,
                              ...
                              keypoint_method, variation_number)) #Process 是 multiprocessing 模块中的一个类，用于创建一个新进程
            p.start()
            processes.append(p)#processes.append(p) 将启动的进程对象添加到 processes 列表中，以便后续管理

        for p in processes:
            p.join() #阻塞主进程，直到对应的子进程完成
    #函数没有返回值是因为它的主要目的是通过多进程并行地修改共享的 ReplayBuffer 对象
