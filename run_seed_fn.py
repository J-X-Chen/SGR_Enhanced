...
from agents import sgr_enhanced
...
def run_seed(rank, cfg: DictConfig, obs_config: ObservationConfig, cams,
    ...
    if cfg.method.name == 'PERACT_BC':
    ...
    elif cfg.method.name == 'SGR_Enhanced':
        replay_buffer = sgr_enhanced.launch_utils.create_replay(
            cfg.replay.batch_size, cfg.replay.timesteps,
            replay_path if cfg.replay.use_disk else None, cams,
            cfg.method.voxel_sizes, cfg.rlbench.camera_resolution)
        #agents文件下
        #import rpdb; rpdb.set_trace()
        sgr_enhanced.launch_utils.fill_multi_task_replay(
            cfg,
            obs_config,#obs_config.left_shoulder_camera.depth ---> False
            rank,
            replay_buffer,#这里面存着图片
            tasks,
            cfg.rlbench.demos,
        ...
        #import rpdb;rpdb.set_trace(port=4444)
        agent = sgr_enhanced.launch_utils.create_agent(cfg)
    ...
    wrapped_replay = PyTorchReplayBuffer(replay_buffer,
                                         num_workers=cfg.framework.num_workers)#wrapped_replay.dataset() ---> Dataloader
